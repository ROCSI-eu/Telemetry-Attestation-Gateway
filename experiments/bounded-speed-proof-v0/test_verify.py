...
class FakeVerifierTests(unittest.TestCase):
    ...
    def test_verifier_reconstructs_expected_inputs_without_witness(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp); self._fake_bb(d, 1500)
            artifact, proof_path, vk = d / "public.cbor", d / "proof", d / "vk"
            artifact.write_bytes(common.BOUNDED.encode_public_artifact(1500)); proof_path.write_bytes(b"synthetic proof"); vk.write_bytes(b"synthetic vk")
            with patch.dict(os.environ, {"PATH": f"{d}:{os.environ.get('PATH', '')}"}): result = verify.verify_public_package(artifact, proof_path, vk)
            self.assertEqual(result["input"]["status"], "ACCEPTED"); self.assertEqual(result["cryptographic"]["status"], "VALID"); self.assertNotIn("valid", result); self.assertEqual(result["relying_party_decision"], "NOT_MADE")

    def test_altered_public_limit_changes_verifier_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp); self._fake_bb(d, 1500)
            artifact, proof_path, vk = d / "public.cbor", d / "proof", d / "vk"
            artifact.write_bytes(common.BOUNDED.encode_public_artifact(1501)); proof_path.write_bytes(b"synthetic proof"); vk.write_bytes(b"synthetic vk")
            with patch.dict(os.environ, {"PATH": f"{d}:{os.environ.get('PATH', '')}"}): result = verify.verify_public_package(artifact, proof_path, vk)
    ...
    def test_malformed_public_artifact_stops_before_tool_lookup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp); artifact, proof_path, vk = d / "public.cbor", d / "proof", d / "vk"
            artifact.write_bytes(b"\xa0"); proof_path.write_bytes(b"proof"); vk.write_bytes(b"vk")
            with patch.dict(os.environ, {"PATH": ""}): result = verify.verify_public_package(artifact, proof_path, vk)
            self.assertEqual(result["input"]["status"], "REJECTED"); self.assertEqual(result["cryptographic"]["status"], "NOT_CHECKED")

    def test_missing_proof_is_unverifiable_not_invalid(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp); artifact = d / "public.cbor"; artifact.write_bytes(common.BOUNDED.encode_public_artifact(1500))
            result = verify.verify_public_package(artifact, d / "missing-proof", d / "missing-vk")
            self.assertEqual(result["input"]["status"], "ACCEPTED"); self.assertEqual(result["cryptographic"]["status"], "UNVERIFIABLE")
...
class BoundaryTests(unittest.TestCase):
    def test_verifier_has_no_speed_argument(self) -> None:
        ...
        self.assertNotIn("--speed-cm-s", source); self.assertNotIn("Prover.toml", source)

    def test_typed_dimensions_remain_separate(self) -> None:
        result = verify._base_result()
        for dimension in ("cryptographic", "policy", "freshness", "revocation", "replay", "assurance", "publication", "relying_party_decision"): self.assertIn(dimension, result)
        self.assertNotIn("overall_valid", result); self.assertNotIn("business_disposition", result)

    def test_over_limit_is_rejected_before_tooling(self) -> None:
        with patch.object(prove, "require_prover_tools") as tools: result = prove.prove_package(1501, 1500, Path("/tmp/should-not-be-written-tag-test"))
        tools.assert_not_called(); self.assertEqual(result["predicate"], "NOT_SATISFIED"); self.assertEqual(result["cryptographic"], "NOT_ATTEMPTED"); self.assertFalse(result["package_written"])

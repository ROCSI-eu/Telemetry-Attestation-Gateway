...
from common import BOUNDED, LABELS, require_prover_tools, sha256_file
...
def capture_evidence(output: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tag-proof-evidence-") as tmp:
        ...
        if below_verify["cryptographic"]["status"] != "VALID":
            raise RuntimeError("below-limit synthetic proof did not verify")
        if equal_verify["cryptographic"]["status"] != "VALID":
            raise RuntimeError("equality-boundary synthetic proof did not verify")
        over_result = prove_package(1501, 1500, over)
        if over_result["predicate"] != "NOT_SATISFIED" or over_result["package_written"]:
            raise RuntimeError("over-limit synthetic witness was not rejected before proving")
        if over.exists() and any(over.iterdir()):
            raise RuntimeError("over-limit rejection left an output package")
        ...
        if tampered_verify["cryptographic"]["status"] != "INVALID":
            raise RuntimeError("mutated proof was not rejected")
        ...
        if altered_public_verify["cryptographic"]["status"] != "INVALID":
            raise RuntimeError("altered canonical public material was not rejected cryptographically")
        ...
        if malformed_verify["input"]["status"] != "REJECTED" or malformed_verify["cryptographic"]["status"] != "NOT_CHECKED":
            raise RuntimeError("malformed public material did not fail before cryptographic verification")
        ...
        if escaped:
            raise RuntimeError(f"private witness artifacts escaped prover workspace: {escaped}")
        ...
        for package in (below, equal):
            ...
            if manifest.get("private_witness_disclosed") is not False:
                raise RuntimeError("proof package does not record the witness-disclosure boundary")
            if "speed_cm_s" in collect_keys(manifest):
                raise RuntimeError("manifest contains an unexpected private speed field")
        ...
        result = {
            ...
            "cases": {
                "below_limit": _dimension_summary(below_verify),
                "equality_boundary": _dimension_summary(equal_verify),
                "over_limit": {"input": "ACCEPTED", "predicate": "NOT_SATISFIED", "cryptographic": "NOT_ATTEMPTED", "package_written": False},
                "tampered_proof": _dimension_summary(tampered_verify),
                "altered_public_input": _dimension_summary(altered_public_verify),
                "malformed_public_artifact": _dimension_summary(malformed_verify),
            },
            "package_evidence": {"proof_size_bytes": (below / "proof").stat().st_size, "proof_sha256": sha256_file(below / "proof"), "verification_key_sha256": sha256_file(below / "vk"), "public_artifact_sha256": sha256_file(below / "public-artifact.cbor"), "private_witness_disclosed": False, "verifier_accepts_witness_input": False},
